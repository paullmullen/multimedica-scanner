...
describe("supported display update operation", () => {
  test("uses a dedicated parameter set and exact asset allowlist", () => {
    const source = fs.readFileSync(path.join(__dirname, "..", "provision-scanner.ps1"), "utf8");
  ...
  test("ships a root updater with atomic swap, health verification, and rollback", () => {
    const source = fs.readFileSync(
      path.join(__dirname, "..", "bootstrap", "display-update.js"),
      "utf8"
    );
    expect(source).toContain("validateBundle");
    expect(source).toContain("fs.renameSync(target, backup)");
    expect(source).toContain("fs.renameSync(staged, target)");
    expect(source).toContain("DISPLAY_UPDATE_ROLLED_BACK");
    expect(source).toContain("multimedica-display.service");
