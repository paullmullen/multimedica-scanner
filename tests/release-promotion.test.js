...
function fixture(options = {}) {
  ...
  const manager = createReleaseManager({
    ...
    productionHealthRequester: async (url) => {
      ...
      const health =
        typeof options.productionHealth === "function"
    ...
    postPromotionVerifier: async (value) => {
      calls.verifiers.push(value);
      return options.verifier === undefined ? { ok: true } : options.verifier;
    },
    rollbackVerifier: async () =>
      options.rollbackVerifier === undefined ? { ok: true } : options.rollbackVerifier,
    switchCurrent: (target, transactionId) => {
      const temporary = path.join(
        path.dirname(path.join(root, "current")),
        `.current.${transactionId}.tmp`
      );
...
async function stage(fixtureValue) {
  const staged = await fixtureValue.manager.stageArtifact({
    artifactPath: fixtureValue.artifact.artifactPath,
    expectedSha256: fixtureValue.artifact.sha256,
    version: VERSION,
  });
...
describe("release promotion and automatic rollback", () => {
  ...
  test("blocks promotion before candidate health and promotes first activation atomically", async () => {
    value = fixture();
    const staged = await value.manager.stageArtifact({
      artifactPath: value.artifact.artifactPath,
      expectedSha256: value.artifact.sha256,
      version: VERSION,
    });
    await expect(value.manager.promoteCandidate(staged.transactionId)).rejects.toThrow(
      "release promotion failed"
    );
    const transaction = stateStore.readJson(
      `releases/transactions/${staged.transactionId}.json`,
      value.stateRoot
    );
    ...
    expect(fs.realpathSync(path.join(value.root, "current"))).toBe(
      path.join(value.releaseRoot, VERSION)
    );
    ...
    expect(stateStore.readJson("installed-version.json", value.stateRoot)).toMatchObject({
  ...
  test("replaces an existing release and restores it after production health failure", async () => {
    value = fixture({
      productionHealth: () =>
        value.calls.service.length <= 2
          ? {
              statusCode: 503,
              body: { service: "multimedica-production", ok: false, state: "starting" },
            }
          : {
              statusCode: 200,
              body: { service: "multimedica-production", ok: true, state: "healthy" },
    ...
    stateStore.writeJson(
      "installed-version.json",
      {
        current_version: "5.2.2-existing",
        current_dir: previous,
        current_symlink: path.join(value.root, "current"),
        previous_version: null,
        previous_dir: null,
        last_known_good_version: "5.2.2-existing",
        last_known_good_dir: previous,
        last_activation_at: BUILT_AT,
        last_activation_txn: "old",
      },
      value.stateRoot
    );
    const transactionId = await stage(value);
    ...
    expect(stateStore.readJson("installed-version.json", value.stateRoot).current_version).toBe(
      "5.2.2-existing"
    );
    expect(
      stateStore.readJson(`releases/transactions/${transactionId}.json`, value.stateRoot).stage
    ).toBe("rolled_back");
  });
  ...
  test("rejects wrong production port, missing verifier, and outside current target", async () => {
    ...
    await expect(value.manager.promoteCandidate(outsideId)).rejects.toThrow(
      "release promotion failed"
    );
  ...
  test("disables production on first activation failure and records rollback_failed when restoration fails", async () => {
    ...
    await expect(value.manager.promoteCandidate(firstId)).rejects.toThrow(
      "release promotion rolled back"
    );
    expect(value.calls.service).toContain("disable");
    expect(
      stateStore.readJson(`releases/transactions/${firstId}.json`, value.stateRoot).stage
    ).toBe("first_activation_failed");
    ...
    expect(value.calls.service).toContain("disable");
    expect(
      stateStore.readJson(`releases/transactions/${failedId}.json`, value.stateRoot).stage
